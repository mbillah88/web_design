...
def profile_update(request, user_id):
    ...
    if not (request.user.is_superuser or request.user.id == target_user.id):
        return HttpResponseForbidden("আপনার এই প্রোফাইল আপডেট করার অনুমতি নেই")
    ...
    if request.method == "POST":
        ...
        target_user.save()
        return HttpResponseRedirect(f"/accounts/profile/{user_id}/")
...
def profile_form_view(request, user_id):
    ...
    return render(request, 'accounts/profile_form.html', {'form': ProfileForm(instance=user)})
def profile_form_view(request, user_id):
...
def add_department(request):
    if request.method == 'POST':
        ...
        if not name or not code:
            return JsonResponse({'success': False, 'error': 'নাম ও কোড আবশ্যক'})
...
def add_department1(request):
    if request.method == 'POST':
        name = request.POST.get('name', '').strip()
        if not name:
            return JsonResponse({'success': False, 'error': 'ডিপার্টমেন্টের নাম আবশ্যক'})
...
def module_group_list_view(request):
    ...
    return render(request, 'accounts/modals/module_group.html', {
...
def module_group_delete(request, id):
    if request.method == "POST":
        ...
        group.delete()
        return redirect('accounts:module_list')  # ✅ redirect to module list page

    return redirect('accounts:module_list')  # fallback for non-POST