...
class Payment(models.Model):
    ...
    created_by = models.ForeignKey(CustomUser, related_name="created_payments", on_delete=models.SET_NULL, null=True)
    updated_by = models.ForeignKey(CustomUser, related_name="updated_payments", on_delete=models.SET_NULL, null=True)

    def __str__(self):
        return f"{self.type.title()} Payment ৳{self.amount} → {self.reference}"
...
class Expense(models.Model):
    title = models.CharField(max_length=100)  # e.g. "Electricity Bill"
    ...
    def __str__(self):
        return f"{self.title} ৳{self.amount} ({self.type.name})"
