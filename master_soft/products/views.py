...
def purchase_summary(request, pk):
    ...
    # ✅ Updated: Use PurchasePayment instead of generic Payment
    payments = PurchasePayment.objects.filter(
        content_type=content_type,
        object_id=purchase.id,
        is_deleted=False  # Optional: skip deleted payments
    ...
    discount = Decimal(purchase.total_discount)
    paid = Decimal(purchase.paid)
    net_total = total - discount
    due = net_total - paid
...
# 🔧 Utility: Get model by type
def get_payment_model(payment_type):
    return {
        "purchase": PurchasePayment,
        "sale": SalePayment,
        "expense": ExpensePayment,
...
# 🔧 Utility: Log audit
def log_payment_action(instance, action, user):
    PaymentAudit.objects.create(
        payment_type=instance.__class__.__name__,
        payment_id=instance.id,
        action=action,
        performed_by=user,
        changes={
            "amount": float(instance.amount),
            "method": instance.method.name if instance.method else None,
            "note": instance.note,
        }
    )
...
def create_payment(request):
    ...
    try:
        ...
        if not all([payment_type, object_id, content_type_id]):
            return JsonResponse({"success": False, "message": "Missing required fields"}, status=400)

        model_cls = get_payment_model(payment_type)
        if not model_cls:
            return JsonResponse({"success": False, "message": "Invalid payment type"}, status=400)

        content_type = ContentType.objects.get(id=content_type_id)
        reference_obj = content_type.get_object_for_this_type(id=object_id)
        ...
        for method_id, amount, note in zip(methods, amounts, notes):
            ...
            payment = model_cls.objects.create(
                method=method,
                amount=amt,
                date=timezone.now().date(),
                note=note,
            ...
            # Optional: Add purchase-specific fields
            if payment_type == "purchase":
                payment.invoice_number = getattr(reference_obj, "invoice_no", "")
                payment.supplier = getattr(reference_obj, "supplier", None)
                payment.save()

            log_payment_action(payment, "created", request.user)
            total_paid += amt

        # Update reference model's paid field
        if hasattr(reference_obj, "paid"):
            reference_obj.paid += total_paid
...
# ✅ Soft Delete Payment
@csrf_exempt
def delete_payment(request, payment_type, payment_id):
    model_cls = get_payment_model(payment_type)
    if not model_cls:
        return JsonResponse({"success": False, "message": "Invalid payment type"}, status=400)

    payment = get_object_or_404(model_cls, pk=payment_id, is_deleted=False)
    payment.soft_delete(request.user)
    log_payment_action(payment, "deleted", request.user)

    # Optional: deduct from reference model
    reference_obj = payment.content_object
    if hasattr(reference_obj, "paid"):
        reference_obj.paid -= payment.amount
        reference_obj.updated_by = request.user
        reference_obj.save()
