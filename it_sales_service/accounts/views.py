...
def module_group_create(request):
    if request.method == 'POST':
...
def module_group_update(request, id):
    group = get_object_or_404(ModuleGroup, pk=id)
    if request.method == 'POST':
        form = ModuleGroupForm(request.POST, instance=group)
        if form.is_valid():
            group = form.save()
            return JsonResponse({'success': True, 'group': {'id': group.id, 'name': group.name}})
        return JsonResponse({'success': False, 'error': form.errors}, status=400)
...
def module_group_list_partial(request):
    groups = ModuleGroup.objects.all().order_by('order')
    return render(request, 'accounts/partials/module_group_list.html', {
        'module_groups': groups
    })